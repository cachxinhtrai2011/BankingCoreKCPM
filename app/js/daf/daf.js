...
(function () {
    ...
    var _window = window,
        ...
        _touch, _host, _odp, _geolocation, _locationWatchId, _storage;
    ...
    _app.upload = function (method, options) {
        ...
        function configureClear(hasValue) {
            ...
            if (container.is('.app-clearing')) {
                ...
                $a('app-clear ui-btn ui-btn-icon-notext ui-icon-trash ui-corner-all').appendTo(container).text(resourcesFiles.Clear).attr('title', resourcesFiles.Clear);
            else
                ...
                if (field.OnDemandStyle === 2) {
                    ...
                    container.on('dragenter', function () {
                        ...
                        if (!container.is('.app-uploading')) {
                            if (_touch)
                                _app.input.focus({ field: container.closest('[data-field]').data('field') });
            ...
            $(files).each(function (index) {
                ...
                if (uploadSupport.fileReader && file.type.match(/^image\//i)) {
                    ...
                    reader.onload = function (event) {
                        $htmlTag('img').attr('src', event.target.result).insertAfter(this._fileInfo).one('load', function () {
                            if (options.change)
                                options.change();
                        });
                        this._fileInfo = null;
