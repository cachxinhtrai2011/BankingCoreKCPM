...
(function () {
    ...
    _app.display = function (options) {
        ...
        function flowChanged() {
            ...
            if (restoreScrollTop) {
                centerContainer = $(document.elementFromPoint(
        ...
        if (options === 'designer')
            if (_app.display.designer)
                _app.display.designer.attach();
            else
                //$('<script></script>').attr('src', _app.resolveClientUrl('~/js/daf/touch-flow.' + _app.jsExt()))
                //    .appendTo('head');
                $.ajax({
                    type: 'GET',
                    dataType: 'script',
                    cache: true,
                    url: _app.resolveClientUrl('~/js/daf/touch-flow.' + _app.jsExt())
                })
                    .then(function () {
                        _app.display.designer.attach();
                    });
