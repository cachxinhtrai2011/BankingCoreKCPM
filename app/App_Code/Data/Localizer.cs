...
namespace GiaPhuocSME.Data
{
    ...
    public class Localizer
    {
        ...
        public static Stream CreateDictionaryStream(string culture, string baseName, string objectName, out string[] files)
        {
            ...
            var result = t.Assembly.GetManifestResourceStream(CultureManager.ResolveEmbeddedResourceName(string.Format("GiaPhuocSME.{0}{1}", baseName, fileName), culture));
            if (result == null)
                result = t.Assembly.GetManifestResourceStream(CultureManager.ResolveEmbeddedResourceName(string.Format("GiaPhuocSME.{0}", fileName), culture));
            if (result == null)
            {
        ...
        public static LocalizationDictionary CreateDictionary(string baseName, string objectName)
        {
            ...
            var fileName = string.Format("GiaPhuocSME.{0}.{1}.txt", objectName, culture);
