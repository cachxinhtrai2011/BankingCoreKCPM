...
namespace GiaPhuocSME.Handlers
{
    ...
    public class SiteBase : GiaPhuocSME.Web.PageBase
    {
        ...
        protected virtual void InitializeSiteMaster()
        {
            ...
            var themePath = Server.MapPath("~/App_Themes/GiaPhuocSME");
            if (Directory.Exists(themePath))
                foreach (var stylesheetFileName in Directory.GetFiles(themePath, "*.css"))
                {
                    var fileName = Path.GetFileName(stylesheetFileName);
                    if (!(fileName.Equals("_Theme_Social.css")))
                    {
                        var link = new HtmlLink()
                        {
                            Href = ("~/App_Themes/GiaPhuocSME/" + fileName)
                        };
