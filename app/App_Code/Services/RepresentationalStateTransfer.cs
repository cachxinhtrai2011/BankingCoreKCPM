...
using GiaPhuocSME.Data;
using GiaPhuocSME.Handlers;
using GiaPhuocSME.Security;
        ...
        protected virtual void CombineScripts(HttpContext context, bool isSaaS, string scriptName, string culture, string version)
        {
            ...
            if (isSaaS)
            {
                if (ApplicationServices.IsTouchClient)
                    ...
                    sb.AppendFormat(string.Format("$(\'<link></link>\').appendTo($(\'head\')).attr({{ href: \'{0}/App_Themes/GiaPhuocSME/" +
                                "_Theme_Social.css?{0}\', type: \'text/css\', rel: \'stylesheet\' }});", ApplicationServices.Version), baseUrl);
                try
                {
                ...
                sb.AppendFormat("var __targetFramework=\'4.7.2\';__tf=4.0;__cothost=\'appfactory\';__appInfo=\'BankingC" +
                        "ore|{0}\';", BusinessRules.JavaScriptString(context.User.Identity.Name));
        ...
        protected virtual void BeginResponsePadding(HttpRequest request, StreamWriter sw)
        {
            ...
            else
            {
                if ((request.HttpMethod == "GET") && UriRestConfig.IsJSONPRequest(request))
                {
                    ...
                    sw.Write("GiaPhuocSME=typeof GiaPhuocSME==\'undefined\'?{{}}:GiaPhuocSME;GiaPhuocSME.{0}=", instance);
                }
            }
        }
